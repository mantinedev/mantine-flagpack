import React from 'react';
import { Box, packSx } from '@mantine/core';
import type { FlagProps } from '../types';

export function SGFlag({ size = 40, radius, sx, ...others }: FlagProps) {
  return (
    <Box
      sx={[
        (theme) => ({
          display: 'inline-block',
          overflow: 'hidden',
          lineHeight: 1,
          width: size,
          borderRadius: theme.fn.radius(radius),

          '& svg': {
            display: 'block',
          },
        }),
        ...packSx(sx),
      ]}
      {...others}
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 24">
        <defs>
          <mask id="mantine-856wmmj3d" fill="#fff">
            <path fillRule="evenodd" d="M0 0h32v24H0z" />
          </mask>
          <mask id="mantine-2g5ogww2g" fill="#fff">
            <use fillRule="evenodd" xlinkHref="#mantine-4q4ilr6dn" />
          </mask>
          <path id="mantine-4q4ilr6dn" d="M0 0h32v24H0z" />
        </defs>
        <g fill="none" fillRule="evenodd" mask="url(#mantine-856wmmj3d)">
          <use fill="#f7fcff" xlinkHref="#mantine-4q4ilr6dn" />
          <path fill="#e31d1c" d="M0 0h32v12H0z" mask="url(#mantine-2g5ogww2g)" />
          <path
            fill="#f1f9ff"
            d="M8.868 2.195s-2.825 1.1-2.825 4.179 2.825 4.216 2.825 4.216c-1.428.795-5.017 0-5.017-4.216s3.644-4.526 5.017-4.179zm1.535 5.286l.387.891h.912l-.673.754.202.955-.828-.49-.81.49.184-.955-.675-.715.914-.039zm2.946-.055l.387.892h.913l-.674.753.203.955-.829-.49-.81.49.185-.955-.675-.715.914-.038zM9.37 5.112l.388.892h.912l-.673.753.202.955-.828-.49-.81.49.184-.955-.675-.715.914-.038zm5.36-.044l.386.892h.912l-.673.753.203.955-.829-.49-.81.49.184-.955L13.427 6l.914-.04zM12.097 3.17l.387.892h.912l-.673.753.202.955-.828-.49-.81.49.184-.955-.675-.715.914-.038z"
            mask="url(#mantine-2g5ogww2g)"
          />
        </g>
      </svg>
    </Box>
  );
}
